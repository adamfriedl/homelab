...
| Creates `raw_*` table schemas when needed | Inserts rows into `raw_*` |
| Grants BQ IAM via Terraform | Uses ADC / your user creds |
| — | Creates `stg_*` / `mart_*` via SQL in DAG tasks |

Project and dataset IDs are configured in DAG env vars (`GCP_PROJECT_ID`, `BQ_DATASET_ID`) and must match Terraform outputs.
...
- **`docs/observability-warehouse.md`** — future homelab metrics in BigQuery
- **`pipelines/README.md`** — local Airflow scaffold
